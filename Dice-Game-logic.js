import { playerCount, addPlayer, deleteNewestPlayer } from "./PLayer";
...
function calculateLowerSectionScore(playerId, rowIndex) {
  ...
  switch (rowIndex) {
    case 9: for (let i = 6; i >= 1; i--) if (counts[i] >= 2) { score = i * 2; break; } break;
    case 10: {
      let pairs = [];
    ...
    case 11: for (let i = 6; i >= 1; i--) if (counts[i] >= 3) { score = i * 3; break; } break;
    case 12: for (let i = 6; i >= 1; i--) if (counts[i] >= 4) { score = i * 4; break; } break;
    case 13: {
      let three = 0, pair = 0;
      for (let i = 6; i >= 1; i--) {
    ...
    case 14: if ([1, 2, 3, 4, 5].every(i => counts[i])) score = 15; break;
    case 15: if ([2, 3, 4, 5, 6].every(i => counts[i])) score = 20; break;
    case 16: score = diceValues.reduce((acc, val) => acc + val, 0); break;
    case 17: if (counts.some(c => c === 5)) score = 50; break;
  }
...
// Exports
export { moveToNextPlayer, checkIfGameFinished };